...
try:
    ...
    HEIC_SUPPORT = False
    print("⚠️ Warning: pillow-heif not installed. HEIC images from iPhone will not be supported.")
...
class MoovEZVisionAnalyzerV5:
    """Gemini Vision API based Analyzer V5 - Supports both images and videos"""
    ...
    def _get_vision_prompt(self):
        ...
        return f"""
You are a specialized AI assistant for analyzing images and videos of rooms and furniture for a moving company called MoovEZ.
...
4. Include SIZE HINTS in the name (e.g., "queen bed", "large sectional sofa", "4-drawer dresser")
...
7. Count boxes/bins separately (name them as "box" or "storage bin")
...
"""
    ...
    def analyze_multiple_files(self, file_paths):
        """Analyze multiple images and/or videos using a single API call with File API"""
        ...
        try:
            ...
            for file_path in file_paths:
                if self._is_video_file(file_path):
                    ...
                    print(f"⚠️ Warning: Unsupported file type: {os.path.basename(file_path)}")
            ...
            # Upload and add all files to the contents
            uploaded_files = []
            ...
            for video_path in video_files:
                ...
                try:
                    ...
                    print(f"   ❌ Error loading/uploading video: {e}")
                    continue
            ...
            print(f"\n🤖 Sending request to Gemini with {len(uploaded_files)} file(s)...")
            ...
            self.metrics["api_calls"] = 1  # Only one API call for all files
    ...
    def display_results(self, items_data, calculations):
        ...
        print("\n" + "="*80)
        print("🏠 MOOVEZ MOVING ANALYSIS RESULTS (V5 - Gemini + JSON Database + Video)")
        print("="*80)
    ...
    def display_metrics(self):
        ...
        print("\n" + "="*80)
        print("⏱️ PERFORMANCE METRICS (V5 - Gemini with File API + Video Support)")
        print("="*80)
        ...
        print(f"Gemini API Calls: {self.metrics.get('api_calls', 1)} (single call for all files)")
    ...
    def process_moving_request(self, file_paths, pickup_access, dropoff_access, travel_time=30):
        """
        Main method to process a moving request with image/video analysis
        
        Args:
            file_paths: List of paths to image/video files or a single file path string
        ...
        # Step 1: Analyze all files (Gemini Vision - items only, no dimensions/weight)
        items_data = self.analyze_multiple_files(valid_paths)
        ...
        final_result = {
            ...
            "features": ["image_support", "video_support", "multi_file_upload"]
        }
...
def get_user_input():
    ...
    # Get file paths (images and/or videos)
    print("🖼️ MEDIA FILES (Images and/or Videos):")
    print("You can enter multiple file paths separated by commas")
    print("Supported formats:")
    print("  Images: .jpg, .jpeg, .png, .gif, .bmp, .webp, .tiff, .heic, .heif")
    print("  Videos: .mp4, .mov, .avi, .mkv, .webm, .flv, .wmv, .m4v")
    print("Example: image1.jpg, video1.mp4, image2.png, photo.heic")
    file_input = input(f"Enter file path(s) (or press Enter for default 'test.jpg'): ").strip()
...
def main():
    ...
    print("🚛 Welcome to MoovEZ Moving Bot V5 - Powered by Gemini + JSON Database + Video Support")
    print("="*80)
    ...
    if result:
        ...
        print(f"📸 Images Processed: {result['imagesProcessed']}")
        print(f"📹 Videos Processed: {result['videosProcessed']}")
