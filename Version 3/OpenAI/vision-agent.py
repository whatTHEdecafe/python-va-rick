...
class MovingCalculatorV2:
    ...
    def __init__(self, items_file='moving_items_logistics_v2.json', rules_file='moving_calculation_rules.json'):
    ...
    def calculate_total_logistics(self, items, pickup_access, dropoff_access, travel_time=30):
        ...
        Args:
            items: List of items from OpenAI Vision analysis (name, quantity, size hints, location)
...
class MoovEZVisionAnalyzerV4:
    """OpenAI Assistants API based Vision Analyzer V4 - Uses JSON data for volume/weight"""
    
    def __init__(self):
        """Initialize the MoovEZ Vision Analyzer V4 with Assistants API"""
        ...
        print(f"✅ Assistant V4 initialized: {self.assistant.id}")
    
    def _get_or_create_assistant(self):
        """Get existing assistant or create a new one"""
        assistant_name = "MoovEZ Moving Item Analyzer V4"
...
Your task is to identify all movable items in the provided images. DO NOT estimate dimensions, volume, or weight - these will be looked up from our database.
...
IMPORTANT RULES:
1. Include SIZE HINTS in the name (e.g., "queen bed", "large sectional sofa", "4-drawer dresser")
2. Do NOT estimate dimensions, volume, or weight - we have this data in our database
3. Do NOT calculate anything - just identify items
4. Count boxes/bins separately (name them as "box" or "storage bin")
5. Be specific with furniture types (use aliases like "loveseat", "sectional", "armoire", etc.)
...
"""
    ...
    def start_timer(self):
        ...
        print(f"🕐 Starting MoovEZ Vision Analysis V4 at {time.strftime('%H:%M:%S')}")
        ...
        try:
            ...
            message = self.client.beta.threads.messages.create(
                ...
                content=[
                    {
                        "type": "text",
                        "text": "Analyze this image and identify all movable items with size hints. Return the results as JSON following the specified format."
                    },
                    {
                        "type": "image_file",
                        "image_file": {
    ...
    def analyze_multiple_images(self, image_paths):
        ...
        for idx, image_path in enumerate(image_paths, 1):
            ...
            if result[0]:  # If analysis was successful
                ...
                combined_summary['totalItems'] += summary.get('totalItems', 0)
                combined_summary['totalBoxes'] += summary.get('totalBoxes', 0)
                
                # Track the highest clutter level
                clutter = summary.get('clutterLevel', 'low')
                if clutter == 'high' or combined_summary['clutterLevel'] == 'high':
                    combined_summary['clutterLevel'] = 'high'
                elif clutter == 'moderate':
                    combined_summary['clutterLevel'] = 'moderate'
                
                if summary.get('notes'):
                    combined_summary['notes'].append(f"Image {idx}: {summary['notes']}")
        ...
        # Combine notes
        combined_summary['notes'] = ' | '.join(combined_summary['notes']) if combined_summary['notes'] else 'No specific notes'
    ...
    def display_results(self, items_data, calculations):
        ...
        print("\n" + "="*80)
        print("🏠 MOOVEZ MOVING ANALYSIS RESULTS (V4 - JSON Database)")
        print("="*80)
    ...
    def display_metrics(self):
        ...
        print("\n" + "="*80)
        print("⏱️ PERFORMANCE METRICS (V4)")
        print("="*80)
        ...
        print(f"OpenAI API Calls: {self.metrics.get('api_calls', 1)} (for item identification only)")
    ...
    def process_moving_request(self, image_paths, pickup_access, dropoff_access, travel_time=30):
        ...
        # Step 1: Analyze all images (OpenAI Assistants API - items only, no dimensions/weight)
...
def main():
    """Main function to run the MoovEZ Vision Analyzer V4 with Assistants API"""
    print("🚛 Welcome to MoovEZ Moving Bot V4 - Powered by OpenAI Assistants API + JSON Database")
    print("="*80)
    
    # Initialize the analyzer
    try:
        analyzer = MoovEZVisionAnalyzerV4()
    ...
    if result:
        ...
        output_file = "moving_analysis_result_v4.json"
