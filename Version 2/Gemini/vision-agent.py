...
class MovingCalculator:
    ...
    def calculate_total_logistics(self, items, pickup_access, dropoff_access, travel_time=30):
        ...
        Args:
            items: List of items from Gemini AI analysis
        ...
        selected_vehicle = self.available_trucks[0]
        ...
        workers_needed = max(2, min(3, int(weight_with_buffer / 1250) + 1))
        ...
        return {
            ...
            'material': {
                ...
                'vehicle': {
                    ...
                    'title': selected_vehicle['title'],
                    'reason': f"Selected based on volume ({volume_with_buffer:.1f} cu ft) and weight ({weight_with_buffer:.1f} lbs)"
                }
            },
            'volume': {
...
class MoovEZVisionAnalyzer:
    def __init__(self):
        """Initialize the MoovEZ Vision Analyzer"""
    ...
    def get_analysis_prompt(self):
        """Get the prompt for identifying items in images - NO calculations"""
        return f"""
Analyze the provided images and identify all movable items relevant to a moving process. Your task is to ONLY identify items and their physical characteristics - DO NOT perform any time or cost calculations.

For each item, provide the following details:
- Image ID: The ID provided with the image analyzed
- Image Name: The name of the image provided
- Number Of Items: Count of identical items (e.g., 2 identical chairs)
- Item Name/Type: Descriptive name or category (e.g., "sofa", "bookshelf", "dining chair")
- Dimensions/Size: Estimate width, height, and depth in inches
- Weight Estimate: Rough weight estimate in pounds
- Requires Disassembly: Does the item need disassembly for transport? (true/false)
- Fragile: Is the item fragile and requires extra care? (true/false)
- Bulkiness: Is it bulky, compact, or regular?
- Location: Where is the item located (e.g., "living room", "bedroom")
- Movable: Can it be moved by a moving company? (true/false)
- Prohibited: Is it on the prohibited items list? (true/false)
- Pack in Box: Should it be packed in a box? (true/false)

Prohibited Items List:
{json.dumps(self.prohibited_items)}

IMPORTANT: Focus ONLY on identifying items and their physical characteristics. Do NOT calculate:
- Time estimates
- Moving costs
- Number of movers needed
- Vehicle requirements

Return your output as strictly valid JSON following this template:
...
{{
  ...
  "logistics": {{
    ...
    "notes": "general observations about space and accessibility"
  }}
}}
...
"""
    ...
    def analyze_image(self, image_path):
        """Analyze a single image using Gemini - identify items only"""
        ...
        try:
            ...
            return items_data, analysis_time
    ...
    def analyze_multiple_images(self, image_paths):
        ...
        for idx, image_path in enumerate(image_paths, 1):
            ...
            if result[0]:  # If analysis was successful
                items_data, analysis_time = result
    ...
    def calculate_moving_logistics(self, items_data, pickup_access, dropoff_access, travel_time=30):
        """Calculate moving logistics using Python and JSON files - NO Gemini AI"""
        ...
        try:
            # Use Python calculator instead of Gemini
    ...
    def display_results(self, items_data, calculations):
        ...
        print("\n" + "="*80)
        print("🏠 MOOVEZ MOVING ANALYSIS RESULTS")
        print("="*80)
        ...
        if calculations:
            ...
            # Vehicle and Workers
            vehicle = calculations.get('material', {}).get('vehicle', {})
            print(f"Vehicle Type: {vehicle.get('title', 'Unknown')}")
            print(f"Reason: {vehicle.get('reason', 'N/A')}")
            print(f"Number of Movers: {calculations.get('material', {}).get('numberOfWorkers', 0)}")
            print()
            
            # Volume and Weight
            volume = calculations.get('volume', {})
            weight = calculations.get('weight', {})
            print(f"Total Volume: {volume.get('totalCubicFeet', 0)} cu ft (with buffer: {volume.get('withBuffer', 0)} cu ft)")
            print(f"Total Weight: {weight.get('totalPounds', 0)} lbs (with buffer: {weight.get('withBuffer', 0)} lbs)")
            print()
    ...
    def display_metrics(self):
        ...
        print(f"Gemini API Calls: {self.metrics.get('api_calls', 1)} (for item identification)")
    ...
    def process_moving_request(self, image_paths, pickup_access, dropoff_access, travel_time=30):
        ...
        # Step 1: Analyze all images (Gemini AI - items only)
...
def main():
    """Main function to run the MoovEZ Vision Analyzer"""
    print("🚛 Welcome to MoovEZ Moving Bot - Powered by Gemini 2.0 + Python Calculator")
    print("="*60)
    
    # Initialize the analyzer
    try:
        analyzer = MoovEZVisionAnalyzer()
    ...
    if result:
        ...
        output_file = "moving_analysis_result.json"
