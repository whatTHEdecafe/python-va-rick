...
def build_vision_prompt(calculator: MovingCalculator) -> str:
    """Build the Gemini vision prompt from the calculator's item database."""
    categories_list = []
    for category in calculator.items_data["categories"]:
        cat_name = category["category"]
        aliases = ", ".join(category.get("aliases", []))
        logic = category.get("classificationLogic", {})
        size_hints = (
            f"Small: {logic.get('small', '')}, "
            f"Medium: {logic.get('medium', '')}, "
            f"Large: {logic.get('large', '')}"
        )
    ...
    return f"""
You are analyzing images and videos of rooms and furniture for a moving company. Your task is to identify all movable/furniture items in the provided media files (images and/or videos). Multiple files may show different rooms or angles of the same space. DO NOT estimate dimensions, volume, or weight - these will be looked up from our database. AVAILABLE ITEM CATEGORIES (with size classification hints):
{categories_reference}
...
CATALOG CATEGORIES (use these names and aliases when identifying items):
{categories_reference}
...
class GeminiClient(BaseAIClient):
    ...
    def get_vision_prompt(self) -> str:
        ...
        return build_vision_prompt(self._calculator)
