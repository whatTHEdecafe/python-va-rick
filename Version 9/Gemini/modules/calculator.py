...
class MovingCalculator:
    ...
    *_FRICTION constants below.
    ...
    def _category_from_catalog_row(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Build one catalog category from a CSV-style or normalized API row."""
        canonical_item = self._safe_text(row.get('CanonicalItem') or row.get('canonicalItem'))
        if not canonical_item:
            return None

        raw_aliases = row.get('Aliases') if row.get('Aliases') not in (None, '') else row.get('aliases')
        if isinstance(raw_aliases, list):
            alias_parts = [self._safe_text(alias) for alias in raw_aliases if self._safe_text(alias)]
        else:
            alias_parts = [
                self._safe_text(alias)
                for alias in self._safe_text(raw_aliases).split(';')
                if self._safe_text(alias)
            ]
        if canonical_item.lower() not in {alias.lower() for alias in alias_parts}:
            alias_parts.insert(0, canonical_item)

        source_id = row.get('Id') if row.get('Id') not in (None, '') else row.get('id')
        ...
        return {
            ...
            "twoPersonRequired": self._parse_two_person(row.get('TwoPersonFlag') or row.get('twoPersonFlag')),
            "stackable": self._parse_bool(row.get('StackableFlag') or row.get('stackableFlag')),
            "stackableSavings": self._parse_float(row.get('StackableSavingsPct') or row.get('stackableSavingsPct')),
            "elevatorFit": self._safe_text(row.get('FitsElevatorHint') or row.get('fitsElevatorHint')),
            "sourceId": self._safe_text(source_id),
            "createdAt": self._safe_text(row.get('CreatedAt') or row.get('createdAt')),
            "updatedAt": self._safe_text(row.get('UpdatedAt') or row.get('updatedAt')),
            ...
            "unloadMultiplier": {
                "mainFloor": self._parse_float(
                    row.get('UnloadMultiplierMainFloor') or row.get('unloadMultiplierMainFloor')
                ),
                "elevator": self._parse_float(
                    row.get('UnloadMultiplierElevator') or row.get('unloadMultiplierElevator')
                ),
                "stairs": self._parse_float(
                    row.get('UnloadMultiplierStairs') or row.get('unloadMultiplierStairs')
                ),
            },
            "notes": self._safe_text(
                row.get('ClassificationLogicNotes') or row.get('classificationLogicNotes')
            ),
        }
    ...
    def _load_csv_items_data(self, csv_path: str) -> Dict[str, Any]:
        ...
        with open(csv_path, 'r', newline='', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                category = self._category_from_catalog_row(row)
                if category:
    ...
    @classmethod
    def build_items_data_from_catalog_rows(
        cls,
        rows: List[Dict[str, Any]],
        *,
        source_label: str = "backend-api",
    ) -> Dict[str, Any]:
        """Build in-memory catalog data from normalized CSV/API rows."""
        calc = cls.__new__(cls)
        categories = []
        for row in rows or []:
            category = calc._category_from_catalog_row(row)
            if category:
                categories.append(category)
        return {
            "metadata": {
                "description": "Moving logistics database loaded from backend API",
                "version": "backend-api-runtime",
                "source": source_label,
                "units": {
                    "volume": "cubic feet",
                    "weight": "pounds",
    ...
    def __init__(
        ...
        """Initialize calculator with JSON, CSV, or in-memory item database data."""
        ...
        self.items_file = os.path.abspath(items_file) if items_file else "backend-api://VisionItems"
        self.items_data = {}
    ...
    def _get_hour_range_data(self, total_hours: float):
        ...
        try:
            ...
            first_supported_hours = min(row['estimate_hours'] for row in rows)

            if total_hours < first_supported_hours:
                return total_hours, total_hours
